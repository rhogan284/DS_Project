...
SCHEMA = ('Flight_ID:STRING, Flight_Status:STRING, Aircraft_ID:STRING, Destination_Airport:STRING, '
          'Departure_Airport:STRING, Departure_Time:TIMESTAMP, Arrival_Time:TIMESTAMP, Passport_No:STRING, '
          'Model_ID:STRING, Seat_No:INTEGER')
...
def truncate_destination_airport(element):
    max_length = 3  # adjust based on your column's actual length
    element['Destination_airport'] = element['Destination_airport'][:max_length]
    return element
...
def run(input_subscription, output_table):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        messages = (
                ...
                | "Truncate Destination Airport" >> beam.Map(truncate_destination_airport)  # Insert this line
